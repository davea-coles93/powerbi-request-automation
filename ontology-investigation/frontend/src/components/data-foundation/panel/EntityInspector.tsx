...
import { useRelationships } from '../../../hooks/useOntology';
...
export function EntityInspector({ data }: EntityInspectorProps) {
  const { attributes, entities } = useDataFoundationData();
  const selectNode = useDataFoundationStore((s) => s.selectNode);
  const attrConfig = entityTypeConfig.attribute;
  const entityConfig = entityTypeConfig.entity;

  const { data: relationships, isLoading: relsLoading } = useRelationships(data.id);
  ...
  // Resolve related entities from relationships
  const relatedEntities = (relationships ?? []).map((rel) => {
    const isFrom = rel.from_entity_id === data.id;
    const relatedEntityId = isFrom ? rel.to_entity_id : rel.from_entity_id;
    const relatedEntity = entities.find((e: any) => e.id === relatedEntityId);
    return {
      ...rel,
      direction: isFrom ? 'outgoing' as const : 'incoming' as const,
      relatedEntity,
      relatedEntityId,
    };
              ...
              <button
                ...
                onClick={() => {
                  if (rel.relatedEntity) {
                    selectNode({ id: rel.relatedEntityId, type: 'entity', data: rel.relatedEntity });
                  }
                }}
                className="w-full text-left px-3 py-2 rounded-lg border border-gray-100 hover:border-gray-200 hover:bg-gray-50 transition-colors group"
              >
                <div className="flex items-center gap-2">
                  <span
                    className="flex-shrink-0 w-5 h-5 rounded flex items-center justify-center text-xs"
                    style={{ backgroundColor: entityConfig.bg, color: entityConfig.color }}
                  >
                    {entityConfig.icon}
                  </span>
                  <span className="text-sm text-gray-700 group-hover:text-gray-900 truncate">
                ...
                <div className="text-xs text-gray-400 mt-0.5 pl-7">
                  {rel.relationship_type.replace('_', '-to-')}
                  {!rel.is_active && (
                    <span className="ml-1.5 text-amber-500">(inactive)</span>
                  )}
                  {rel.source === 'tmdl' && (
                    <span className="ml-1.5 text-indigo-400">TMDL</span>
                  )}
                </div>
              </button>
            ))}
